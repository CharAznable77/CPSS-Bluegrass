...
bool pump_on = false; //Pump is nominally off
int throttlePin = 36; //pin connected to motor controller to tell motor to spin
// int goPin = 34; // pin that determines whether pump is on or off
...
// Define the pump output pin for the signal
// const int throttlePin = 10;  // Connect to the pin you want to output the signal on
// const int potPin = 36; //output pin on test stand
// const int pulseInterval = 20000; // 20 ms in microseconds - ensure we get 50 Hz
...
void setup() {
  ...
  Serial.println("Initializing pump motor");
  pinMode(throttlePin, OUTPUT);  // Set the throttle pin as an output
  //analogWriteFrequency(throttlePin, 50000); //uncomment this for PWM
...
void init_relay_board() // https://www.tindie.com/products/bugrovs2012/16-channel-i2c-electromagnetic-relay-module-iot/
{
  ...
  if(!relay_module.isConnected())
  {
    ...
    relay_module.write(2, HIGH); // turn on dump valve
  }
  unsigned char i;
...
void pump()
{
  ...
  //PUMP TESTING
    ...
    digitalWrite(throttlePin, HIGH);       // Start the pulse
      ...
      // // Apply PWM command
      // int pulseWidth = 2000;//map(r_df.pwm_duty_cycle, 0, 1023, 1000, 2000); // Map 0-100% to 1000-2000 µs
      ...
      // // Serial.printf("Pulse sent: Width=%d µs\n", pulseWidth);
      // // // Wait for the rest of the 20 ms period
      // // //delay(pulseInterval - pulseWidth / 1000);
  }else{
...
void receive_command_packet() {
  ...
  if (packetSize) {
   ...
   // Serial.println();
    ...
    // int pinState = digitalRead(goPin);
    ...
    if (r_df.relay_states[9] == HIGH) //valve relay pin number (may change)
      {
        pump_on = true;
        Serial.println("On");
      }
